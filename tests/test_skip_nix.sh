...
assert_ignored_with_skip_nix() {
    ...
    local expected_paths=(
        ...
        ".chezmoiscripts/07b_mise-install.sh"
...
assert_not_ignored_without_skip_nix() {
    ...
    if printf '%s\n' "$rendered" | grep -q '^\.chezmoiscripts/07b_mise-install\.sh$'; then
        echo "did not expect 07b_mise-install.sh in .chezmoiignore when skipNix=false" >&2
        exit 1
    fi
