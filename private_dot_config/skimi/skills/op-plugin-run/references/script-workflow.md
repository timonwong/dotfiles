...
3. If no TTY, run direct in current shell (skip tmux).
4. If tmux unavailable, run direct fallback in current shell.
5. If tmux available with TTY:
   - ensure session `op-auth`
   - run command in a new window in `op-auth`
   - attach/switch to `op-auth`
6. If tmux session/window creation fails, fallback direct.
